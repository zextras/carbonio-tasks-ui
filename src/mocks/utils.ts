 ...
 * makes it assignable to the operation result types generated by typescript-operations v6 (where
 * nullable fields are `T | null` rather than the optional `T | null | undefined` of schema types).
 */
...
export function mockUpdateTaskStatus(
	...
	// Apollo needs `__typename` at runtime to normalize the result into the cache. v6 operation
	// result types no longer model it (the document doesn't select it), so we add it back and cast.
